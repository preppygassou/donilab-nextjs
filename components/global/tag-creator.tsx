...
const TagCreator = ({ getSelectedTags, hubId, defaultTags }: Props) => {
  ...
  useEffect(() => {
    if (hubId) {
      const fetchData = async () => {
        const response = await getTagsForHub(hubId)
  ...
  const handleAddTag = async () => {
    ...
    setSelectedColor('')
    try {
      const response = await upsertTag(hubId, tagData)
      toast({
  ...
  const handleDeleteTag = async (tagId: string) => {
    ...
    try {
      ...
      toast({
        title: 'Deleted tag',
        description: 'The tag is deleted from your Hub.',
      })
