 ...
 const { data: site, isLoading, error } = useSite("dml");
  ...
  return (
    ...
    <ExpertiseContainer>
        ...
        <ExpertiseHero>
        <ExpertiseheroparalaxeLeft src={"/assets/svg/aboutheroleftparalaxe.svg"} alt=""/>
        <ExpertiseheroparalaxeRight src={"/assets/svg/aboutherorightparalaxe.svg"} alt=""/>
      </ExpertiseHero>

      <AllExpertisesSection>
        <FakeMarging>
      <ExpertiseSection expertises={site?.expertise} expertise/>
      {
        site?.expertise.map((expertise,index)=>(
        index === 0 ? <Acceleration>
        <ContentInfo>
        <object  type="image/svg+xml" width="100" height="100" data={expertise.icon.url} className="svg"> 
        </object>
        <h1>{expertise.title[locale]}</h1>
        <p>{expertise.description[locale]}</p>
        <h4>
